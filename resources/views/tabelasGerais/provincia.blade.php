        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"> Lista de Provincias </li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
<!--                        <button>  <a class="btn btn-success ml-5" href="javascript:void(0)" id="createNewsem"> Criar</a> </button>-->
         ...
         <div class="card-body">
           <table class="table table-condensed data-table stripe">
                <thead>
                    <tr>
                        <th width="5%">No</th>
                         <th>Código</th>
                        ...
                        <form id="ItemForm" name="ItemForm" class="form-horizontal">
                           <input type="hidden" name="Item_id" id="Item_id">
                            <div class="form-group">
                                ...
                                <div class="col-sm-12">
                                    <input type="text" class="form-control" id="nomeProvincia" name="nomeProvincia" placeholder="" value="" maxlength="50" required="">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-12">
                                    <textarea id="descricao" name="descricao" required="" placeholder="" class="form-control"></textarea>
                                </div>
                            </div>
...
<script type="text/javascript">
  $(function () {
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('provincia.index') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'id', name: 'id'},
            {data: 'nomeProvincia', name: 'nomeProvincia'},
    ...
    $('#createNewItem').click(function () {
        ...
        $('#modelHeading').html("Adicionar Provincia");
        $('#ajaxModel').modal('show');
    });
    
    $('body').on('click', '.editItem', function () {
      var Item_id = $(this).data('id');
      $.get("{{ route('provincia.index') }}" +'/' + Item_id +'/edit', function (data) {
          $('#modelHeading').html("Editar Provincia");
          $('#saveBtn').val("edit-user");
          $('#ajaxModel').modal('show');
          $('#Item_id').val(data.id);
          $('#nomeProvincia').val(data.nomeProvincia);
    ...
    $('body').on('click', '.deleteItem', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('provincia.store') }}"+'/'+Item_id,
            success: function (data) {
                table.draw();
            },
            error: function (data) {
...
<script>
   ...
   $('select[name=distrito_id]').on('change', function () {
         var distrito = $('#distrito_id').find(":selected").text();//pega a provincia
         //alert(distrito);
         var distritoo=$('input[name=distrito]'); 
          distritoo.val(distrito);
      var selected = $(this).find(":selected").attr('value');
      $.ajax({
